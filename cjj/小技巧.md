...
```javascript
  ...
  const num2cn = num => {
      ...
      return n.replace(/\d/g, (n, i) => `${cnstr.charAt(n)}${unit.charAt(i)}`)     .replace(/零(仟|佰|拾|角)/g, "零")
        .replace(/(零)+/g, "零")
...
返回能设置scrollTop的页面根元素
```javascript
  const getSCrollHtmlElement = () => {
    const top = document.body.scrollTop
    if (top === 0) {
      document.body.scrollTop += 1
      if (document.body.scrollTop === 0) return document.documentElement
